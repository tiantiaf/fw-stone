...
CY_ISR(CyBLE_Uart_Interrupt)
{
    ...
    uint8 receivedData[BLE_STONE_HAL_Uart_FIFO_SIZE] = {0u};

    /* Evaluate RX interrupt event */
    srcInterrupt = BLE_STONE_HAL_Uart_GetRxInterruptSourceMasked();

    if(0u != (srcInterrupt & BLE_STONE_HAL_Uart_INTR_RX_NOT_EMPTY))
    {
        if(0u != (srcInterrupt & BLE_STONE_HAL_Uart_INTR_RX_PARITY_ERROR))
        {
            uartRxStatus |= CyBLE_INTR_RX_PARITY_ERROR;
        }
        if(0u != (srcInterrupt & BLE_STONE_HAL_Uart_INTR_RX_FRAME_ERROR))
        {
            uartRxStatus |= CyBLE_INTR_RX_FRAME_ERROR;
        }
        if(0u != (srcInterrupt & BLE_STONE_HAL_Uart_INTR_RX_OVERFLOW))
        {
            uartRxStatus |= CyBLE_INTR_RX_OVERFLOW;
        }
        if(uartRxStatus == CyBLE_INTR_RX_SUCCESS)
        {
            length = (uint8)BLE_STONE_HAL_Uart_SpiUartGetRxBufferSize();
            for(srcCount = 0u; srcCount < length; srcCount++)
            {
                receivedData[srcCount] = (uint8)BLE_STONE_HAL_Uart_SpiUartReadRxData();
            }
        }
        else
        {
            BLE_STONE_HAL_Uart_SpiUartClearRxBuffer();
        }
        for(uartTxStatus = 0u; uartTxStatus < length; uartTxStatus++)
        {
            CyBLE_pf_handle_uart_rx_data(receivedData[uartTxStatus]);
        }
        BLE_STONE_HAL_Uart_ClearRxInterruptSource(srcInterrupt);
    }
    else
    {
    ...
    srcInterrupt = BLE_STONE_HAL_Uart_GetTxInterruptSourceMasked();

    /* Stack manager TX uart complete */
    if(0u != (srcInterrupt & BLE_STONE_HAL_Uart_INTR_TX_UART_DONE))
    {
        if(0u != (srcInterrupt & BLE_STONE_HAL_Uart_INTR_TX_OVERFLOW))
        {
            /*Stack manager TX uart error */
            uartTxStatus |= CyBLE_INTR_TX_OVERFLOW;
        }
        CyBLE_pf_handle_uart_tx_complt();
        BLE_STONE_HAL_Uart_ClearTxInterruptSource(srcInterrupt);
    }
    else
    {
