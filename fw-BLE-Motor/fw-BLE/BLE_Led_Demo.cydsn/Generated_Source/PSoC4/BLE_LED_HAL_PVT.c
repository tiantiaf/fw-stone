...
{
    /* Setup ISR */
    BLE_LED_uart_isr_StartEx(&CyBLE_Uart_Interrupt);

    BLE_LED_HAL_Uart_Start();
}
...
void CyBLE_Uart_Stop (void)
{
    ...
    BLE_LED_uart_isr_Stop();

    BLE_LED_HAL_Uart_Stop();
}
...
{
    BLE_LED_HAL_Uart_SpiUartPutArray(dataBuf, (uint32)length);
}
...
/* Interface to BLE_LED_bless_isr_Enable function */
void CyBleHal_EnableGlobalInterrupts(void)
{
    BLE_LED_bless_isr_Enable();
}

/* Interface to BLE_LED_bless_isr_Disable function */
...
void CyBle_HalInit(void)
{
    ...
    BLE_LED_bless_isr_SetVector(&CyBLE_Bless_Interrupt);
...
{
    BLE_LED_bless_isr_StartEx(&CyBLE_Bless_Interrupt);
}
